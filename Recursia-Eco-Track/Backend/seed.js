...
async function seedDatabase() {
  try {
    ...
    const driverUsers = createdUsers.filter(user => user.role === 'driver');
    const driverProfiles = [];
    
    for (const driverUser of driverUsers) {
      const driver = new Driver({
        user: driverUser._id,
        driverId: `DRV${Date.now().toString().slice(-6)}`, // Generate unique driver ID
        vehicle: {
          type: 'truck',
    ...
    logger.info(`📊 Summary:
      ...
      - Driver Profiles: ${driverProfiles.length}`);
