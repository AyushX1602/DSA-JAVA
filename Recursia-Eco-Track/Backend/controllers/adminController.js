...
// @desc    Clear old test data (pickups older than 1 day)
// @route   DELETE /api/admins/test-data
// @access  Private (Admin)
const clearTestData = async (req, res) => {
  try {
    ...
    logger.info(`Admin ${req.user.id} cleared ${result.deletedCount} old test pickups`);

    res.json({
      success: true,
      message: `Successfully deleted ${result.deletedCount} old test pickups`,
      data: {
