...
const createPickupRequest = async (req, res) => {
  try {
    ...
    const pickup = new Pickup({
      ...
      priority: wasteDetails.type === 'hazardous' ? 'high' : 'normal'
    });
    ...
    try {
      await createNotification({
        ...
        message: `Your ${pickup.wasteDetails.type} waste pickup request has been created successfully.`,
        priority: 'normal',
...
const getUserPickups = async (req, res) => {
  try {
    ...
    // Only show pickups from the last 7 days to avoid old test data
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
    query['scheduling.requestedAt'] = { $gte: sevenDaysAgo };
